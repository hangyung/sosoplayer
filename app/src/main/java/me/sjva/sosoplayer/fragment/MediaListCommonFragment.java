...
import me.sjva.sosoplayer.widget.FileListLongClickDialog;
...
import me.sjva.sosoplayer.activity.OnMainEventListener;
    ...
    public class MediaListCommonFragment extends Fragment implements OnCommonEventListener,
  ...
  public MediaListCommonFragment() {
    rootDir = null;
    subDir = null;
    this.storageInfo = null;
    onMainEventListener = null;
  }
  ...
  private void loadMediaList() {
    if (getActivity() == null || storageInfo == null)
      return;
    rootDir = storageInfo.getPath();
  ...
  private void startPlayer(ArrayList<FileInfo> fileInfos, int postion) {
    if (getActivity() != null && getActivity() instanceof MainActivity) {
      final MainActivity activity = (MainActivity) getActivity();
      activity.startPlayer(fileInfos, postion, false);
    }
  }
  ...
  public void onPlexItemSelect(ArrayList<Video> videos,  int postion) {
    if (getActivity() != null && getActivity() instanceof MainActivity) {
      final MainActivity activity = (MainActivity) getActivity();
      activity.startPlexPlayer(videos, postion, false);
    }
  }
  ...
  public void onPlexItemLongSelect(ArrayList<Video> videos,  int postion) {
    new FileListLongClickDialog(getActivity(), videos, postion, this).show();
  }
  ...
  @Override
  public void onPlayFromScratch(Parcelable parcelable) {
    if (getActivity() != null && getActivity() instanceof MainActivity) {
      final MainActivity activity = (MainActivity) getActivity();
      activity.playFromScratch(parcelable);
    }
  }
  ...
  private boolean LoadMediaStoreFileInfo() {
    ...
    try (Cursor cursor = getActivity().getContentResolver().query(
      ...
      while (cursor.moveToNext()) {
        ...
        builder.setCurrentMediaKey(FileInfo.getExtraUri(storageInfo, contentUri.toString()).toString());
...
//      builder.setDuration(duration);
        ...
        builder.setCurrentMediaKey(FileInfo.getExtraUri(storageInfo, curFile.getAbsolutePath()).toString());
        if (!curFile.isDirectory() ) {
    ...
    for (ExtraStreamFileInfo extraStreamFileInfo : list) {
      ...
      if (extraStreamFileInfo.isDirectory() ||
        ...
        if (storageType == StorageType.Ftp) {
          ...
          builder.setCurrentMediaKey(FileInfo.getExtraUri(storageInfo, childDir.getPath()).toString());
        } else {
          builder.setPath(extraStreamFileInfo.getPath());
          builder.setCurrentMediaKey(FileInfo.getExtraUri(storageInfo, extraStreamFileInfo.getPath()).toString());
        }
  ...
  public void onItemLongSelect(ArrayList<FileInfo> fileInfos, int postion) {
    new FileListLongClickDialog(getActivity(), fileInfos, postion, this).show();
  }
