...
import com.google.android.exoplayer2.ext.ffmpeg2.FfmpegDemuxer;
...
/* package */ final class SosoMediaPeriod
    implements MediaPeriod,
        ExtractorOutput,
        Loader.Callback<SosoMediaPeriod.ExtractingLoadable>,
...
//    this.progressiveMediaExtractor = new BundledExtractorsAdapter(extractorsFactory);
    this.ffmpegDemuxer = new FfmpegDemuxer();
        ...
        () -> {
          if (!released) {
            Assertions.checkNotNull(callback)
                .onContinueLoadingRequested(SosoMediaPeriod.this);
          }
        };
  ...
  private void startLoading() {
    ExtractingLoadable loadable =
        new ExtractingLoadable(
            uri, dataSource,  ffmpegDemuxer, /* extractorOutput= */ this, loadCondition);
    if (prepared) {
  ...
  private final class SampleStreamImpl implements SampleStream {
    ...
    public boolean isReady() {
      return SosoMediaPeriod.this.isReady(track);
    }

    @Override
    public void maybeThrowError() throws IOException {
      SosoMediaPeriod.this.maybeThrowError(track);
    }

    @Override
    public int readData(FormatHolder formatHolder, DecoderInputBuffer buffer,
        boolean formatRequired) {
      return SosoMediaPeriod.this.readData(track, formatHolder, buffer, formatRequired);
    }

    @Override
    public int skipData(long positionUs) {
      return SosoMediaPeriod.this.skipData(track, positionUs);
    }
...
//    @Nullable private TrackOutput icyTrackOutput;
    ...
    public ExtractingLoadable(
      ...
      this.ffmpegDemuxer = ffmpegDemuxer;
